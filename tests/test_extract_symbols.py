"""Extractor logic that reads the elaborated AST.

The functions below take slang symbols and read attributes off them. They are
exercised here with stand-ins that have the same attributes, which pins the
interface handling - the part that distinguishes this tool from a text scanner -
without a full elaboration.
"""
...
def test_interface_connection_prefers_the_source_text():
    """`ifaceConn` names the interface *type* for a boundary port, which would
    merge unrelated streams; the written net name is the correct one."""
    pc = PortConnection(
...
def test_output_connection_follows_the_assignment():
    """An output port wraps its connection in an assignment; the external net is
    on one side of it."""
