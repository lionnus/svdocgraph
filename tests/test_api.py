...
import rtldoc
from rtldoc import api, project

pytestmark = [needs_pyslang]


def test_the_package_gives_the_public_names():
    assert rtldoc.build_documentation is api.build_documentation
    assert rtldoc.extract_design is api.extract_design
    assert rtldoc.Design().modules == {}
    assert rtldoc.__version__
...
def test_build_documentation_uses_the_settings(project_dir, stub_bender):
    (project_dir / "rtldoc.yml").write_text("name: Fancy\noutput: site\n")
...
def test_a_project_that_bender_cannot_read_raises(tmp_path, stub_bender, monkeypatch):
    ...
    monkeypatch.setattr("rtldoc.bender.collect",
...
def _failed(message: str):
    from rtldoc.bender import BenderInfo
