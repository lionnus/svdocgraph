...
(function () {
  ...
  const saved = localStorage.getItem("rtld-theme");
  ...
  if (tt) tt.addEventListener("click", () => {
    ...
    localStorage.setItem("rtld-theme", next);
  });
  ...
  // ---- the search palette ----
  // assets/search.js sets window.RTLDOC_DATA. A page loads a script from the disk,
  // thus the search also operates over file://, where a page cannot fetch a file.
  let DATA = window.RTLDOC_DATA || { modules: [], packages: [], docs: [], files: [] };
  ...
  document.querySelectorAll(".graph[data-zoom]").forEach((fig) => {
    const svg = fig.querySelector("svg.rtld-graph");
    if (!svg) return;
